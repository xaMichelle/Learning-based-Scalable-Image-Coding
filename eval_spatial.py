...
def evaluate(args):
    ...
    x_half = read_png(args.input_image_half)
    x_half = tf.expand_dims(x_half, 0)
    x_half.set_shape([1, None, None, 3])
    x_half_shape = tf.shape(x_half)

    x_quater = read_png(args.input_image_quater)
    x_quater = tf.expand_dims(x_quater, 0)
    x_quater.set_shape([1, None, None, 3])
    x_quater_shape = tf.shape(x_quater)

    # x_half = tf.image.resize_images(images=x, size=(x_shape[1] / 2, x_shape[2] / 2), method=0)
    # x_quater = tf.image.resize_images(images=x, size=(x_shape[1] / 4, x_shape[2] / 4), method=0)
    # x_half_shape = tf.shape(x_half)
    # x_quater_shape = tf.shape(x_quater)
    ...
    x_B_hat = x_B_hat[:, :x_quater_shape[1], :x_quater_shape[2], :]
    x_e1_hat = x_e1_hat[:, :x_half_shape[1], :x_half_shape[2], :]
    x_e2_hat = x_e2_hat[:, :x_shape[1], :x_shape[2], :]

    num_pixels_B = tf.cast(tf.reduce_prod(tf.shape(x_quater)[:-1]), dtype=tf.float32)
    num_pixels_e1 = tf.cast(tf.reduce_prod(tf.shape(x_half)[:-1]), dtype=tf.float32)
    num_pixels_e2 = tf.cast(tf.reduce_prod(tf.shape(x)[:-1]), dtype=tf.float32)

    eval_bpp_B = tf.reduce_sum(tf.log(y_B_likelihoods)) / (-np.log(2) * num_pixels_B)
    eval_bpp_e1 = tf.reduce_sum(tf.log(y_e1_res_likelihoods)) / (-np.log(2) * num_pixels_e1) + eval_bpp_B
    eval_bpp_e2 = tf.reduce_sum(tf.log(y_e2_res_likelihoods)) / (-np.log(2) * num_pixels_e2) + eval_bpp_e1
    ...
    mse_B = tf.reduce_mean(tf.squared_difference(x_quater, x_B_hat))
    mse_e1 = tf.reduce_mean(tf.squared_difference(x_half, x_e1_hat))
    ...
    msssim_e1 = tf.squeeze(tf.image.ssim_multiscale(x_e1_hat, x_half, 255))
    msssim_e2 = tf.squeeze(tf.image.ssim_multiscale(x_e2_hat, x, 255))

    with tf.Session() as sess:
        latest = tf.train.latest_checkpoint(checkpoint_dir=args.checkpoint_dir)
        tf.train.Saver().restore(sess, save_path=latest)
        tensors_B = [string_B, tf.shape(x_quater)[1:-1], tf.shape(y_B)[1:-1]]
        arrays_B = sess.run(tensors_B)
        tensors_e1 = [string_e1, tf.shape(x_half)[1:-1], tf.shape(y_e1_res)[1:-1]]
        arrays_e1 = sess.run(tensors_e1)
        tensors_e2 = [string_e2, tf.shape(x)[1:-1], tf.shape(y_e2_res)[1:-1]]
        ...
        eval_bpp_B, eval_bpp_e1, eval_bpp_e2= sess.run(
            [eval_bpp_B, eval_bpp_e1, eval_bpp_e2]
        )

        mse_B, mse_e1, mse_e2 = sess.run(
            [mse_B, mse_e1, mse_e2]
        )

        psnr_B, psnr_e1, psnr_e2 = sess.run(
            [psnr_B, psnr_e1, psnr_e2]
        )

        x_B_hat, x_quater = sess.run([x_B_hat, x_quater])
        x_e2_hat, x = sess.run([x_e2_hat, x])
        # msssim_e2 = sess.run(msssim_e2)
        # msssim_e1 = sess.run(msssim_e1)
        # msssim_B = sess.run(msssim_B)
        # msssim_B, msssim_e1, msssim_e2 = sess.run(
        #     [msssim_B, msssim_e1, msssim_e2]
        # )

        num_pixels_B, num_pixels_e1, num_pixels_e2 = sess.run([num_pixels_B, num_pixels_e1, num_pixels_e2])


        bpp_B = len(packed_B.string) *8 / num_pixels_B
        bpp_e1 = len(packed_e1.string) *8 / num_pixels_e1
        bpp_e2 = len(packed_e2.string) *8 / num_pixels_e2

        ebpp_list = [eval_bpp_B, eval_bpp_e1, eval_bpp_e2]
        ebpp_list = [round(i,4) for i in ebpp_list]
        bpp_list = [bpp_B, bpp_e1, bpp_e2]
        bpp_list = [round(i,4) for i in bpp_list]
        mse_list = [mse_B, mse_e1, mse_e2]
        mse_list = [round(i,4) for i in mse_list]
        psnr_list = [psnr_B, psnr_e1, psnr_e2]
...
def decompress(args):
    ...
    tensors = [string_B, x_B_shape, y_B_shape, string_e1, x_e1_shape, y_e1_shape, string_e2, x_e2_shape, y_e2_shape]
    arrays = arrays_B + arrays_e1 + arrays_e2
    ...
    y_B_tilde_up = upconv_e1(y_B_hat)
    ...
    y_e1_hat = tf.concat([y_B_tilde_up, y_e1_hat], -1)
    x_e1_hat = synthesis_transform_e1(y_e1_hat)
    y_e1_tilde_up = upconv_e2(y_e1_hat)
    ...
    y_e2_hat = tf.concat([y_e1_tilde_up, y_e2_hat], -1)
    x_e2_hat = synthesis_transform_e2(y_e2_hat)
    ...
    with tf.Session() as sess:
        ...
        sess.run([op_B, op_e1, op_e2], feed_dict=dict(zip(tensors, arrays)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_image_half', type=str, default='./kodak_half/kodim01.png', help="Kodak half image path.")
    parser.add_argument('--input_image_quater', type=str, default='./kodak_quater/kodim01.png', help="Kodak quater image path.")
